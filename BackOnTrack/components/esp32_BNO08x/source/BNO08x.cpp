...
esp_err_t BNO08x::init_spi()
{
    ...
    {
        // Bus already initialized by another BNO08x instance sharing this SPI host
        // (Back on Track dual-IMU shared-bus design: both IMUs on SPI3_HOST, each
        // with its own CS/INT/RST). Do NOT take ownership of the bus here; just
        // proceed to add this device. The first instance owns and frees the bus.
        ret = ESP_OK;
        init_status.spi_bus = false; // this instance must not free the shared bus on deinit
    }
    else if (ret != ESP_OK)
    {
        // clang-format off
